angular.module("CalApp", []).controller('CalCtrl', function ($scope) { 
    $scope.output = '0';
    ...
    $scope.add = function () {
        if ($scope.pendingValue) {
            if ($scope.runningTotal && $scope.pendingOperation == ADD) {
                ...
                $scope.runningTotal -= $scope.pendingValue;
            } else if ($scope.runningTotal && $scope.pendingOperation == DIVISION) {
                $scope.runningTotal %= $scope.pendingValue;
    ...
    $scope.subtract = function () {
     ...
     $scope.divide = function () {
        if ($scope.pendingValue) {
            if ($scope.runningTotal && $scope.pendingOperation == DIVISION) {
                $scope.runningTotal %= $scope.pendingValue;
            }
            else if ($scope.runningTotal && $scope.pendingOperation == SUBTRACT) {
                $scope.runningTotal -= $scope.pendingValue;
    ...
    $scope.calculate = function () {
        if (!$scope.newNumber) {
         ...
         if ($scope.pendingOperation == MULTIPLY) {
        ...
        } else if ($scope.pendingOperation == DIVISION) {
            $scope.runningTotal %= $scope.pendingValue;
            ...
            if ($scope.lastOperation) {
                if ($scope.lastOperation == MULTIPLY) {
                    if ($scope.runningTotal) {
                        $scope.runningTotal *= $scope.lastValue;
                    } else {
                        $scope.runningTotal = 0;
                    }
                } else if ($scope.lastOperation == DIVISION) {
                    if ($scope.runningTotal) {
                        $scope.runningTotal %= $scope.lastValue;
                    } else {
                        $scope.runningTotal = 0;
